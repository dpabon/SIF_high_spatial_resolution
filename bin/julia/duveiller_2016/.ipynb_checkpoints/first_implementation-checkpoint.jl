...
function gaussianWeightFilter(x)
    ...
    dista = exp.(
        -(((xM .- (scale_fact * 1.5)) .^ 2) .+ ((yM .- (scale_fact * 1.5)) .^ 2)) ./
        (2 * scale_fact^2),
    )
...
function costfun(b, VI, ET, LST, sif_ob)
    ...
    sif_pred =
        vegetation(VI, b[1], b[2]) .* water(ET, b[3], b[4]) .* temperature(LST, b[5], b[6])
...
function sif(VI, ET, LST, b)
    return vegetation(VI, b[1], b[2]) .* water(ET, b[3], b[4]) .*
           temperature(LST, b[5], b[6])
end
...
et = [100.0, 200.0, 120.0, 40.0, 50.0]
lst = [28.0, 30.0, 25.0, 21.0, 28.0]
...
sif_cube = Cube(
    open_dataset(
        "/Net/Groups/BGI/work_3/OEMC/oemc_sif/data/SIF_TROPOMI_gridded_10km_filtered_europe_2018-05-04_2020-12-26.zarr/",
    ),
)
...
sif_cube_low = sif_cube[
    Ti=Between(DateTime("2018-07-01"), DateTime("2018-07-31")),
    Variable=At("SIF_743"),
]
...
lst_cube = Cube(
    open_dataset(
        "/Net/Groups/BGI/work_3/OEMC/oemc_sif/data/LST_Sentinel3_gridded_1km_europe_2018-01-04_2020-12-29.zarr/",
    ),
)
...
lst_cube_high =
    lst_cube[Ti=Between(DateTime("2018-07-01"), DateTime("2018-07-31")), Variable=At("LST")]
...
modis_cube = Cube(
    open_dataset(
        "/Net/Groups/BGI/work_3/OEMC/oemc_sif/data/MC43A4_gridded_1km_europe_2018-05-01_2020-12-31_YAXA.zarr/",
    ),
)
...
ndwi_cube_high = mapCube(
    ndwi,
    (modis_cube_high[Variable=At("4")], modis_cube_high[Variable=At("2")]),
    indims = indims,
    outdims = outdims,
)
...
sentinel_3_cube = Cube(
    open_dataset(
        "/Net/Groups/BGI/work_3/OEMC/oemc_sif/data/OLCI_Sentinel3_gridded_1km_europe_2018-01-04_2020-12-29.zarr/",
    ),
)
...
ogvi_cube_high = sentinel_3_cube[
    Variable=At("OGVI"),
    Ti=Between(DateTime("2018-07-01"), DateTime("2018-07-31")),
]
...
lst_cube_high_july =
    mapCube(temporal_mean, lst_cube_high, indims = indims, outdims = outdims)

ndwi_cube_high_july =
    mapCube(temporal_mean, ndwi_cube_high, indims = indims, outdims = outdims)

ogvi_cube_high_july =
    mapCube(temporal_mean, ogvi_cube_high, indims = indims, outdims = outdims)
...
savecube(
    sif_cube_low_july,
    "/Net/Groups/BGI/work_3/OEMC/oemc_sif/data/sif_cube_low_july.zarr";
    overwrite = true,
)
savecube(
    lst_cube_high_july,
    "/Net/Groups/BGI/work_3/OEMC/oemc_sif/data/lst_cube_high_july.zarr";
    overwrite = true,
)
savecube(
    ogvi_cube_high_july,
    "/Net/Groups/BGI/work_3/OEMC/oemc_sif/data/ogvi_cube_high_july.zarr";
    overwrite = true,
)
savecube(
    ndwi_cube_high_july,
    "/Net/Groups/BGI/work_3/OEMC/oemc_sif/data/ndwi_cube_high_july.zarr";
    overwrite = true,
)
...
function aggregate_space(out, incube; scale_factor, axes)
    ...
    test_out =
        aggregate(mean, test, (X(scale_factor), Y(scale_factor)); skipmissingval = true)
...
function param_optim(
    out,
    sif,
    vegetation,
    water,
    temperature;
    param_ini = [1.0, 2.0, 50.0, 0.0, -295.0, 10.0],
    param_min = [0.5, 0.1, 0, -1.0, -310.0, 1.0],
    param_max = [1.5, 5.0, 500.0, 1.0, -290.0, 50.0],
    min_obs = 21,
    inner_optimizer = LBFGS(),
)
    ...
    if !all(isnan, sif) &&
       !all(isnan, vegetation) &&
       !all(isnan, water) &&
       !all(isnan, temperature)
        if length(filter(!isnan, vec(sif))) >= min_obs &&
           length(filter(!isnan, vec(vegetation))) >= min_obs &&
           length(filter(!isnan, vec(water))) >= min_obs &&
           length(filter(!isnan, vec(temperature))) >= min_obs

            if length(filter(!isnan, vec(sif))) ==
               length(filter(!isnan, vec(vegetation))) &&
               length(filter(!isnan, vec(sif))) == length(filter(!isnan, vec(water))) &&
               length(filter(!isnan, vec(sif))) == length(filter(!isnan, vec(temperature)))
                ...
                test = optimize(
                    b -> costfun(b, vi, agua, lst, sif_ob),
                    param_min,
                    param_max,
                    param_ini,
                    Fminbox(inner_optimizer),
...
indims = (
    InDims(
        MovingWindow(:X, pre_step, after_step),
        MovingWindow(:Y, pre_step, after_step),
...
parameters_cube = mapCube(
    param_optim,
    (
        sif_cube_low_july[X=1:100, Y=1:100],
        ogvi_cube_low[X=1:100, Y=1:100],
        ndwi_cube_low[X=1:100, Y=1:100],
        lst_cube_low[X=1:100, Y=1:100],
    ),
    indims = indims,
    outdims = outdims,
    inner_optimizer = LBFGS(),
...
savecube(
    ...
    "/Net/Groups/BGI/work_3/OEMC/oemc_sif/data/parameters_cube_high_OGVI.zarr/";
    overwrite = true,
)
parameters_cube_high = Cube(
    open_dataset("/Net/Groups/BGI/work_3/OEMC/oemc_sif/data/parameters_cube_high.zarr/"),
)
...
function sif_downscaling(out, vegetation, water, temperature, parameters)
    if !all(isnan, vegetation) &&
       !all(isnan, water) &&
       !all(isnan, temperature) &&
       !all(isnan, parameters)

        if length(filter(!isnan, vec(vegetation))) == length(filter(!isnan, vec(water))) &&
           length(filter(!isnan, vec(vegetation))) ==
           length(filter(!isnan, vec(temperature)))
            ...
            out .= sif(
                mean(filter(!isnan, vegetation)),
                mean(filter(!isnan, water)),
                mean(filter(!isnan, temperature)),
                b_optim,
            )
        else
            out .= NaN32
...
sif_cube_high = mapCube(
    sif_downscaling,
    (ogvi_cube_high_july, ndwi_cube_high_july, lst_cube_high_july, parameters_cube_high),
    indims = indims,
    outdims = outdims,
)
